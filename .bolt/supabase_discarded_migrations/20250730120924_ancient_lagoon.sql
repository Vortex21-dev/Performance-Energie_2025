...
DECLARE
    ...
    site_record RECORD := (SELECT * FROM sites LIMIT 1); -- exemple pour définir site_record
BEGIN
    ...
    LOOP
        -- Exemple : afficher chaque indicateur
        RAISE NOTICE 'Indicateur: %, Processus: %', indicator_record.indicateur, indicator_record.processus;
        -- Ici tu peux insérer ou faire ce que tu veux avec indicator_record
    END LOOP;
END;
$$ LANGUAGE plpgsql;
